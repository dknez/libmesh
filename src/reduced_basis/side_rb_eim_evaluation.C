...
void SideRBEIMEvaluation::set_n_basis_functions(unsigned int n_bfs)
{
  _local_eim_basis_functions.resize(n_bfs);
}

void SideRBEIMEvaluation::decrement_vector(SideQpDataMap & v,
                                           const DenseVector<Number> & coeffs)
{
  LOG_SCOPE("decrement_vector()", "SideRBEIMEvaluation");
      ...
      for (const auto & comp : index_range(v_comp_and_qp))
        for (unsigned int qp : index_range(v_comp_and_qp[comp]))
          for (unsigned int i : index_range(_local_eim_basis_functions))
            {
              // Check that entry (elem_side_pair,comp,qp) exists in _local_eim_basis_functions so that
              // we get a clear error message if there is any missing data
              const auto & basis_comp_and_qp = libmesh_map_find(_local_eim_basis_functions[i], elem_side_pair);
...
void SideRBEIMEvaluation::get_parametrized_function_values_at_qps(
...
Number SideRBEIMEvaluation::get_parametrized_function_value(
...
{
  std::vector<Number> values;
  get_parametrized_function_values_at_qps(pf, elem_id, side_index, comp, values);
...
void SideRBEIMEvaluation::get_eim_basis_function_values_at_qps(unsigned int basis_function_index,
                                                               dof_id_type elem_id,
                                                               unsigned int side_index,
                                                               unsigned int comp,
...
Number SideRBEIMEvaluation::get_eim_basis_function_value(unsigned int basis_function_index,
                                                         dof_id_type elem_id,
                                                         unsigned int side_index,
                                                         unsigned int comp,
...
const SideRBEIMEvaluation::SideQpDataMap &
SideRBEIMEvaluation::get_basis_function(unsigned int i) const
{
  return _local_eim_basis_functions[i];
}

void SideRBEIMEvaluation::add_basis_function_and_interpolation_data(
